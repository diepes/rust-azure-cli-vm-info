...
pub fn get_sku_pricing(
    ...
    //#
    log::info!(
        "Search arm_sku_name='{arm_sku_name}' flex_base_sku_name='{flex_base_sku_name}' NumOpt={}",
    ...
    for (i, price_option) in data.items.iter().enumerate() {
        ...
        if (price_option.sku_name != flex_base_sku_name)
            && (price_option.sku_name != guess_sku_name)
        {
            log::info!("i={i} skip po.sku_name='{po_s}' / po.arm_sku_name='{po_a}' != flex_base_sku_name='{flex_base_sku_name}' guess_sku_name='{guess_sku_name}'  flex_ratio={flex_ratio}"
                        , po_s=price_option.sku_name, po_a=price_option.arm_sku_name
                    );
            continue;
        }
        log::info!("i={i} got Azure pricing po.sku_name='{po_s}' / po.arm_sku_name='{po_a}' == flex_sku='{flex_base_sku_name}' flex_ratio={flex_ratio} {po:#?}'"
        ...
        match (
            ...
            ("Consumption", None, sku)
                if (sku == &guess_sku_name) || (sku == &price_option.sku_name) =>
            {
            ...
            ("Consumption", None, sku)
                if (sku == &format!("{} Spot", guess_sku_name))
                    || (sku == &format!("{} Spot", price_option.sku_name)) =>
            {
            ...
            ("Consumption", None, sku)
                if (sku == &format!("{} Low Priority", guess_sku_name))
                    || (sku == &format!("{} Low Priority", price_option.sku_name)) =>
            {
                // We can have normal or spot or something else
                log::debug!("i={i} skip 'Low Priority' vm, ToDo add to pricing");
            }

            ("Reservation", Some(value), sku)
                if ((sku == &guess_sku_name) || (sku == &price_option.sku_name))
                    && value == "1 Year" =>
            {
