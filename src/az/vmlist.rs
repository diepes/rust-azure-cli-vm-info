...
pub struct VM {
    ...
    pub plan: Option<azure_mgmt_compute::models::Plan>,
...
impl VM {
    pub fn new(
        ...
        // called below in loop with Azure virtual machines retrieved.
        Ok(VM {
            ...
            flex_lookup: None,
            plan: az_vm.plan.clone(), //.expect(&format!("Empty VM plan? name='{:?}'",az_vm.resource.name)),
...
pub async fn get_vms(subscription_id: &str) -> Result<VirtualMachines, Box<dyn std::error::Error>> {
    ...
    while let Some(vms) = vms_pageable.next().await {
        ...
        for vm in vms.value {
            ...
            if let Some(ref plan) = vm.plan {
                println!("VM plan='{:?}'", plan);
            }
